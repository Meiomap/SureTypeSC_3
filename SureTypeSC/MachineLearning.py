...
def create_components_for_gmm_roc(d, threshold, inn):
    ...
    gmm = mixture.GaussianMixture(n_components=6, precisions_init=covariance_ar, covariance_type='full', means_init=means_ar,
                                  weights_init=np.array([float(i)/sum(weights_ar) for i in weights_ar]), max_iter=1000, verbose=0)
...
class GMMcl:
    """Gaussian discriminant analysis - binary classifier where every class is modelled as Gaussian mixture"""
    n_components = None
    ...
    def __init__(self, core, n_components=None, covariance_type='full'):
        ...
        Args:
            core (str): type of gaussian mixture analysis to use, currently 'gmm' for GaussianMixture from the scikit package, otherwise BayesianGaussianMixture
            n_components (int, optional): Number of components per each GMM class. Defaults to None.
            covariance_type (str, optional): Type of covariance matrix to use - {'full', 'tied', 'diag', 'spherical'}. Defaults to 'full'.
        """
    ...
    def fit(self, X, Y):
        """Estimate model parameters per each class

        Args:
            X (np.array): input features, array-like of shape (n_samples, n_features)
            Y (np.array): output labels - array-like of shape (n_samples,) where every element is membership to a class (0 or 1)

        Returns:
            GMMcl: return instance of the class with initialized and fitted GMM per class
        """
        self.classes_ = np.unique(Y)
        self.group_weights = [len(X[Y == i]) / float(len(X))
    ...
    def predict(self, X):
        """Predict labels of binary classification

        Args:
            X (np.array): input feature vector of arra-like shape (n_samples,n_features)

        Returns:
            np.array: vector of labels
        """
    ...
    def predict_proba(self, X):
        ...
        Args:
            X (np.array): input feature vector, array-like shape (n_samples,n_features)

        Returns:
            np.array: vector of scores of length (n_samples,)
        """
        ll = [mix.score_samples(X)for k, mix in enumerate(self.mixtures)]
        #ll = [ mix.score(X[["m","a"]])for k, mix in enumerate(self.mixtures) ]
        ...
        self.group_weights = [
            len(X[binary_predictions == i]) / float(len(X)) for i in self.classes_]
        self.group_log_weights = np.log(self.group_weights)
...
class TrainingDataCreator(object):
    """Class to unify creation of training and testing vectors

    Args:
        object (object): common python object
    """
    def __init__(self, query, exclude_chr=[]):
        """Constructor

        Args:
            query (Data): instance of Data object
            exclude_chr (list, optional): chromosomes to exclude. Defaults to [].
        """
        self._query = query
    ...
    def _run_outliers(x):
        ...
        Args:
            x (np.array): input vector of shape (n_samples,n_features), n_features is commonly 2 (logarithmic mean of intensities m and logarithmic average of intensities a)

        Returns:
            np.array: array of shape (n_samples,n_features + 1 ), the input array is enriched by an extra column indicating outlier score
        """
        
        x['outliers'] = MinCovDet().fit(x[['m_raw', "a_raw"]]).support_
        return x

    def create(self, type="all",  jobid=None, outliers=False, inn=['1', '2'], out="output", ratio=1):
        """Generate training data

        Args:
            type (str, optional): indicates subset of data to include {'hom','het','all'}. Defaults to "all".
            jobid (int, optional): internal job id. Defaults to None.
            outliers (bool, optional): _description_. Defaults to False.
            inn (list, optional): input feature names. Defaults to ['1', '2'].
            out (str, optional): output training feature name (label). Defaults to "output".
            ratio (int, optional):balance dataset or use original data {1 for balance, 0 for keep raw data}. Defaults to 1.

        Raises:
            ValueError: raises Exception in case the dimension of the input vector doesnt correspond the length of the output vector

        Returns:
            tuple: pair of (input feature vector,vector of labels)
        """
        input_feat = inn
        output_feat = out
        ...
        levels_to_stack = [
            i for i in self._query.df.columns.names if i != "feature"]
        ...
        if jobid:
            ...
            protocol.send_data('Dataset properties' + str(Counter(t['gtype'])))

        t = t[input_feat+[output_feat]].dropna()

        t[output_feat] = t[output_feat].astype(int)

        # check for outliers if set
        if outliers:
    ...
    def create_components(self, type="all", outliers=True, jobid=None, inn=['m', 'a'], out='output'):
        """ Calculate initilization parameters for every class (usually binary classification)

        Args:
            type (str, optional): generate GMM only for subset of data {'hom','het','all'}. Defaults to "all".
            outliers (bool, optional): whether to process outliers [NOT IMPLEMENTED]. Defaults to True.
            jobid (_type_, optional): internal job id. Defaults to None.
            inn (list, optional): list of input feature names. Defaults to ['m', 'a'].
            out (str, optional): name of output label. Defaults to 'output'.

        Returns:
        (X, np.array(means), np.array(covariance_ar), np.array([float(i)/sum(weights_ar) for i in weights_ar]), groups)
           tuple: (input feature vector, mean vector per class, covariance matrix per class, weight per class, list of classes)
        """
        input_feat = inn
        output_feat = out
        exclude_chr = self._exclude_chr
...
class Trainer(object):
    """Encapsulates initialization of the classifier and parameter estimation of the model (training)"""
    def __init__(self, data, clfname='gda', inner=['m', 'a'], outer='rf_ratio:1.0_pred', jobid=None, trees=30, outliers=False, ratio=1):
        """ Constructor, initializes training data and classifier

        Args:
            data (_type_): _description_
            clfname (str, optional): name of classifier, currently supported {'rf','dt','lr','kde','knn','bgmm','mlp','vbgmmcl','gmmcl'}. Defaults to 'gda'.
            inner (list, optional): list of inpute feature names. Defaults to ['m', 'a'].
            outer (str, optional): name of output feature. Defaults to 'rf_ratio:1.0_pred'.
            jobid (_type_, optional): internal job id. Defaults to None.
            trees (int, optional): number of trees to use in case of tree classifier. Defaults to 30.
            outliers (bool, optional): process outliers. Defaults to False.
            ratio (int, optional): use balanced dataset for training (1) or keep original data (0). Defaults to 1.
        """
        ...
        if clfname == "gaussian":
            ...
            self._X, self._Means, self._Y, weights, self._components = TrainingDataCreator(
                data).create_components(outliers=outliers, type="all", jobid=jobid)
            self._clf = mixture.GaussianMixture(n_components=len(
                weights), covariance_type='full', means_init=self._Means, precisions_init=self._Y, weights_init=weights, verbose=0)
            ...
            self._X, self._Y = TrainingDataCreator(data).create(outliers=outliers, 
                                                                type="all", 
                                                                jobid=jobid, 
                                                                inn=inner, 
                                                                out=outer, 
        ...
        if clfname == 'rf':
            ...
            self._clf = RandomForestClassifier(
                n_jobs=-1, n_estimators=trees, max_features=None)
        elif clfname == 'dt':
            self._clf = tree.DecisionTreeClassifier()
        elif clfname == 'kde':
            self._clf = (KernelDensity(kernel='gaussian'),
            ...
            self._clf = mixture.BayesianGaussianMixture(
                n_components=7, weight_concentration_prior=0.01)
        elif clfname == "mlp":
    ...
    def train(self, name='rf-gda'):
        ...
        Args:
            name (str, optional): classifier name. Defaults to 'rf-gda'.
        """
    ...
    def predict_self(self):
        """  Function for direct calling predict method of the initialized scikit classifier [NOT REFERENCED]
    ...
    def predict_self_proba(self):
        """  Function for direct calling predict_proba method of the initialized scikit classifier [NOT REFERENCED]
    ...
    def create_roc_data_one_layer(self, test, inn, clftype, selection='all'):
        ...
        # include only AB
        if selection == "all":
            # [Config.D["INPUT_FEATURE"]]
            stacked = stacked[stacked.gtype != "NC"]
        else:
            # [Config.D["INPUT_FEATURE"]]
            stacked = stacked[stacked.gtype == "AB"]
        ...
        if clftype != "gaussian":
            for ind, val in enumerate(self.clf.classes_):
                if val == 1:
                    positive_class = ind
            stacked["_".join(["proba", clftype])] = self.predict_proba(pd.get_dummies(
                stacked[inn]).replace([np.inf, -np.inf], np.nan).replace(np.nan, -200.0))[:, positive_class]
        else:
            gmm_pred = self.clf.predict_proba(stacked[inn].replace(
                [np.inf, -np.inf], np.nan).replace(np.nan, -200.0))
            # positive_gmm=np.take(gmm_pred,np.argwhere(np.array(self._components)!="Null"),axis=1).sum(axis=1)
            positive_gmm = np.take(gmm_pred, np.argwhere(
                np.array(self._components)[:, 1]), axis=1).sum(axis=1)
    ...
    def create_roc_data(self, test, inn, clftype, selection='all'):
        ...
        X = Imputer().fit_transform(pd.get_dummies(
            stacked[inn].replace([np.inf, -np.inf], np.nan)))
        ...
        (components, gmm) = create_components_for_gmm_roc(
            stacked[[lookup, "gtype", "output"] + inn], threshold=0.5)
        ...
        stacked = stacked.assign(pred_gmm_component=np.array(components)[
                                 np.argmax(gmm_pred, axis=1)][:, 0], pred_gmm=np.max(gmm_pred, axis=1))
        # positive_gmm=np.take(gmm_pred,np.argwhere(np.array(components)!="Null"),axis=1).sum(axis=1)
        # positive_gmm=np.take(gmm_pred,np.argwhere(np.array(components)!='Null'),axis=1).sum(axis=1)
        positive_gmm = np.take(gmm_pred, np.argwhere(
            np.array(components)[:, 1] == 1), axis=1).sum(axis=1)
        ...
        stacked["proba_gmm_" +
                clftype] = np.maximum(positive_gmm[:, 0], stacked[lookup].values)
        stacked["proba_positive_gmm_" +
                clftype] = positive_gmm[:, 0]*stacked[lookup]
    ...
    def predict_decorate(self, test, clftype='rf', inn=['m', 'a'], threshold=0.66, selection='all', ratio=1.0):
        ...
        stacked[rec_name] = self.predict(pd.get_dummies(stacked[inn]).replace(
            [np.inf, -np.inf], np.nan).replace(np.nan, -200.0), clftype)
        stacked[rec_name_prob] = self.predict_proba(pd.get_dummies(stacked[inn]).replace(
            [np.inf, -np.inf], np.nan).replace(np.nan, -200.0), clftype)
        ...
        stacked[rec_name] = [
            1 if i > threshold else 0 for i in stacked[rec_name_prob]]
        # else:
        ...
        stacked = stacked.set_index("Chr", drop=True, append=True).unstack(
            level=levels_to_stack).reorder_levels(test.df.columns.names, axis=1).swaplevel(2, 1, axis=0)
        ...
        if rec_name in test.df.columns.get_level_values(level="feature") or rec_name_prob in test.df.columns.get_level_values(level="feature"):
            ...
            test.df.update(stacked.xs(rec_name, axis=1,
                           drop_level=False, level="feature"))
            test.df.update(stacked.xs(rec_name_prob, axis=1,
                           drop_level=False, level="feature"))
            ...
            test.df = test.df.join(stacked.xs(
                rec_name, axis=1, drop_level=False, level="feature"))
            test.df = test.df.join(stacked.xs(
                rec_name_prob, axis=1, drop_level=False, level="feature"))
...
def save_state():
    gdna = Data.create_from_file(
        sys.argv[1], "GDNA", exclude=Settings.Settings.TO_REMOVE)
...
def starting_procedure(decimal='.'):
    ...
    gdna = Data.create_from_frame(pd.read_csv(
        Config.D["GDNA"], sep='\t', decimal=decimal), type='GDNA')
    ...
    sc = Data.create_from_frame(pd.read_csv(
        Config.D["SC"], sep='\t', decimal=decimal), type="SC")
    logging.info("Loaded " + Config.D["SC"])

    for d in [gdna, sc]:
        d.restrict_chromosomes(Config.D["CHROMOSOMES"])
    logging.info("Restricted to chromosomes defined in {} ...".format(
        Config.D["CHROMOSOMES"]))
...
def starting_procedure_T21():
    gdna = Data.create_from_file(
        Config.D["GDNA"], "GDNA", exclude=Settings.Settings.TO_REMOVE + ["GenTrain Score"])
    logging.info("Loaded " + Config.D["GDNA"])
    gdna.apply_NC_threshold_3(0.15, inplace=True)
    logging.info("Applied  threshold to gDNA")
    # gdna.consensus_genotype()

    sc = Data.create_from_file(
        Config.D["SC"], "SC", exclude=["GenTrain Score"])
...
def tm_routine(container, ref, sc, score, run, type):
    sc_transition_matrix, gdna_transition_matrix, raw_transition_matrix = sc.create_transition_matrices(
        ref)
